export default class Service {
    ...
    processFile({query, file, onOcurrenceUpdate, onProgress}) {
        ...
        const elapsed = () => `${Math.round((performance.now() - startAt) / 1000)} secs`
        ...
        return new TransformStream({
            transform(chunk, controller) {
                ...
                for (const line of lines) {
                    ...
                    const currentColumnnItems = line.split(",")

                    for (const columnIndex in currentColumnnItems) {
                        const columnItem = currentColumnnItems[columnIndex]
